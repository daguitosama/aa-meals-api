...
// client data
const filterUserName = body("userName", "bad user name")
  .not()
  .isEmpty()
  .escape();
const fitlerUserPhone = body("userPhone", "bad user phone").isMobilePhone();
const filterUserAddress = body("userAddress", "bad user address")
  .not()
  .isEmpty()
  .escape();

export const singUpFilters = [validateToken];

/** @type RequestHandler */
export async function singUpHandler(req, res, next) {
  var gcRes;
  const { token, form_data } = req.body;
